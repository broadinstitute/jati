...
impl<L, R> Parser for Assign<L, R> where L: Parser<Output=Tree<Raw>>, R: Parser<Output=Tree<Raw>> {
    type Output = (Tree<Raw>, Tree<Raw>);
    fn parse<'a>(&self, input: &crate::input::Input<'a>) -> Result<Success<'a, Self::Output>, ParseIssue> {
